...
char * buffer;
/*实验使用的代码 黄立省*/
int TestTask(void *pParam)
{
    int i,j=0;
	printf("Good morning, dear professor!\n");
	printf("My name is Huang Lisheng.\n");
	printf("Calculating 3 + 2...\n");
	OSTimeDly(10);
	printf("3 + 2 = %d\n", 3 + 2);
	for(;;)
	{
...
void TaskMutex1(void *pParam)
{
	  ...
	  printf("时间%d:高优先级任务TaskMutex1结束运行，删除自己，错误码：%d\n",OSTimeGet(),*perr);	
...
void TaskMutex2(void *pParam)
{
	  ...
	  printf("时间:%d，低优先级任务TaskMutex2结束运行，删除自己，错误码:%d\n",OSTimeGet(),*perr);   
	  OSTaskDel(OS_PRIO_SELF); /*删除本任务*/
	  return;
}
